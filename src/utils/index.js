export function calculateTimeLeft(birthDate, lifeExpectancy) {
  ...
  const expectedDeath = new Date(
    birth.setFullYear(birth.getFullYear() + parseInt(lifeExpectancy))
  );
...
export function getTimePercentage() {
  ...
  const hourPercentage =
    ((now - startOfHour) / (endOfHour - startOfHour)) * 100;
  ...
  const minutePercentage =
    ((now - startOfMinute) / (endOfMinute - startOfMinute)) * 100;
